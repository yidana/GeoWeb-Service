...
import django
django.setup()
from app.models import RegionalPopulationByReligiousAffiliation
import csv

def populate():
    data=csv.reader(open(r'D:\My Stuff\Python\Django\projects\GeoWebService\app\static\app\csv\Population_by_religious_affiliation.csv'),delimiter=',')
    for row in data:
        if row[0] != 'Region':
            religious_affiliation=RegionalPopulationByReligiousAffiliation()
